...
text_random = 'ШКИБИДИ ДОП ДОП ДОП', 'Чем гуще лес, шкибиди доп ес ес', 'Го в доту', \
    'С 8 марта', 'Конспект', 'Абстракт', 'С 8 марта', 'С 8 марта', \
...
def say_text(message):
    ...
    if any(x in text for x in skatilos):
        with open("photo.jpg", "rb") as file:
            f = file.read()
        bot.send_photo(message.chat.id, f, "Скатилась")
        return
    ...
    if message.text == 'Шпанько':
        to_pin = bot.send_message(message.chat.id,
                                  'Номер любимого учителя:\nТатьяны Фёдоровны Шпанько\n+79183792920 звоним не стесняемся!').message_id
        bot.pin_chat_message(chat_id=message.chat.id, message_id=to_pin)
        return

    if message.text == 'Пенсия':
        to_pin = bot.send_message(message.chat.id,
                                  'Номер любимого учителя:\nТатьяны Фёдоровны Шпанько\n+79183792920 звоним не стесняемся!').message_id
        bot.pin_chat_message(chat_id=message.chat.id, message_id=to_pin)
        return

    if message.text == '/shpanko':
        to_pin = bot.send_message(message.chat.id,
                                  'Номер любимого учителя:\nТатьяны Фёдоровны Шпанько\n+79183792920 звоним не стесняемся!').message_id
        bot.pin_chat_message(chat_id=message.chat.id, message_id=to_pin)
        return

    if message.text == '/shpanko@gitler1939_bot':
        to_pin = bot.send_message(message.chat.id,
                                  'Номер любимого учителя:\nТатьяны Фёдоровны Шпанько\n+79183792920 звоним не стесняемся!').message_id
        bot.pin_chat_message(chat_id=message.chat.id, message_id=to_pin)
        return
    ...
    if any(x in text for x in words):
        if any(x in text for x in dogs):
            bot.send_message(message.chat.id, "* сосу член *")

        if any(x in text for x in plac):
            bot.send_message(message.chat.id, '* отсосал этому существу *')

        if any(x in text for x in space):
            bot.send_message(message.chat.id, "* глотаю сперму *")
